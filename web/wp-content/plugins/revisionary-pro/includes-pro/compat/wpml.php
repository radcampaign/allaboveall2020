...
class RevisionaryWPML {
	...
	public static function flt_wpml_posts_request($query) {
		...
		if ( strpos($query, "FROM $wpdb->posts") && ( strpos($query, "'pending-revision'") || strpos($query, ".post_status = 'future-revision'") ) ) {
			...
			if ( strpos($query, 'ELECT') ) {
				...
				if ( preg_match(
					...
					$query = preg_replace( 
						...
						"wpml_translations.language_code = '$1' OR ( $wpdb->posts.post_status IN ('pending-revision', 'future-revision') AND $parent_where )",
						$query
					);	
				}
	...
	public static function flt_wpml_query($query) {
		...
		*/
		...
		if ( strpos($query, "ELECT language_code, COUNT(p.ID)" ) && ! empty($_REQUEST['post_type']) ) {
			$query = str_replace( "ON t.element_id=p.ID", "ON (t.element_id=p.ID OR (t.element_id=p.comment_count AND p.post_status IN ('pending-revision','future-revision')))", $query );
